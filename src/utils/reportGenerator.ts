...
// Экспорт в Word с полной поддержкой русского языка
    ...
    entries.forEach(entry => {
      tableRows.push(
        new TableRow({
          children: [
            new TableCell({
              children: [new Paragraph({ text: formatDateTime(entry.timestamp) })]
            }),
            new TableCell({
              children: [new Paragraph({ text: entry.categoryData?.name || entry.category })]
            }),
            new TableCell({
              children: [new Paragraph({ text: entry.title })]
            }),
            new TableCell({
              children: [new Paragraph({ text: getStatusLabel(entry.status) })]
            }),
            new TableCell({
              children: [new Paragraph({ text: getPriorityLabel(entry.priority) })]
            }),
            new TableCell({
              children: [new Paragraph({ text: entry.author })]
            }),
            new TableCell({
              children: [new Paragraph({ text: entry.equipment?.name || '' })]
            })
          ]
        })
      );
